...
void reg() {
  ...
  XMLLayout.reg(Stack, (node, key) {
    return Stack(
      key: key,
      children: node.children<Widget>(),
      alignment: node.s<AlignmentDirectional>("alignment", AlignmentDirectional.topStart),
      textDirection: node.s<TextDirection>("textDirection"),
  ...
  XMLLayout.reg(Wrap, (node, key) {
    return Wrap(
      ...
      crossAxisAlignment: node.s<WrapCrossAlignment>("crossAxisAlignment", WrapCrossAlignment.start),
      textDirection: node.s<TextDirection>("textDirection"),
      verticalDirection: node.s<VerticalDirection>("verticalDirection", VerticalDirection.down),
    );
  }, mode: XMLLayout.Element);
  ...
  XMLLayout.reg(Semantics, (node, key) {
    return Semantics(
      ...
      onMoveCursorForwardByCharacter: node.s<MoveCursorHandler>("onMoveCursorForwardByCharacter"),
      onMoveCursorBackwardByCharacter: node.s<MoveCursorHandler>("onMoveCursorBackwardByCharacter"),
      onSetSelection: node.s<SetSelectionHandler>("onSetSelection"),
      onDidGainAccessibilityFocus: node.s<VoidCallback>("onDidGainAccessibilityFocus"),
  ...
  XMLLayout.reg(BoxConstraints, (node, key) {
    ...
    if ((a = node.splitMethod("", 4)) != null) {
      ...
      return BoxConstraints.tightFor(width: double.tryParse(a[0]), height: double.tryParse(a[1]));
    } else if ((a = node.splitMethod("tightForFinite", 2)) != null) {
      return BoxConstraints.tightForFinite(width: double.tryParse(a[0]) ?? double.infinity, height: double.tryParse(a[1]) ?? double.infinity);
      ...
      return BoxConstraints.expand(width: double.tryParse(a[0]), height: double.tryParse(a[1]));
    } else {
      return null;
    }
  }, mode: XMLLayout.Text);
    ...
    switch (node.text) {
      ...
      default: {
        ...
        if (a == null) {
          ...
          return AlignmentDirectional(double.tryParse(a[0]), double.tryParse(a[1]));
        }
      }
    }
  }, mode: XMLLayout.Text);
    ...
    switch (node.text) {
      ...
      default: {
        ...
        if ((a = node.splitMethod("fromLTRB", 4)) != null) {
          ...
          return RelativeRect.fromSize(node.v<Rect>(a[0]), node.v<Size>(a[1]));
        } else if ((a = node.splitMethod("fromRect", 2)) != null) {
          return RelativeRect.fromRect(node.v<Rect>(a[0]), node.v<Rect>(a[1]));
        } else return null;
      }
    }
  }, mode: XMLLayout.Text);
