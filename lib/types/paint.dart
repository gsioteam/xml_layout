...
Register reg = Register(() {
  ...
  XmlLayout.reg(Paint, (node, key) {
    if (node.name == "paint") {
      ...
      if ((v = node.s<double>("strokeMiterLimit")) != null)
        paint.strokeMiterLimit = v;
      if ((v = node.s<MaskFilter>("maskFilter")) != null) paint.maskFilter = v;
      if ((v = node.s<FilterQuality>("filterQuality")) != null)
        paint.filterQuality = v;
      if ((v = node.s<Shader>("shader")) != null) paint.shader = v;
      if ((v = node.s<ColorFilter>("colorFilter")) != null)
        paint.colorFilter = v;
      if ((v = node.s<ImageFilter>("imageFilter")) != null)
        paint.imageFilter = v;
  ...
  XmlLayout.reg(ColorFilter, (node, _) {
    ...
    if ((params = node.splitMethod("mode", 2)) != null) {
      return ColorFilter.mode(
          node.v<Color>(params[0]), node.v<BlendMode>(params[1]));
  ...
  XmlLayout.reg(ImageFilter, (node, _) {
    ...
    if ((params = node.splitMethod("blur", 2)) != null) {
      return ImageFilter.blur(
          sigmaX: double.tryParse(params[0]),
          sigmaY: double.tryParse(params[1]));
    } else if ((params = node.splitMethod("matrix", 16)) != null) {
      return ImageFilter.matrix(
          Float64List.fromList(params.map<double>((e) => double.tryParse(e))));
    } else
      return null;
  }, mode: XmlLayout.Text);
});
